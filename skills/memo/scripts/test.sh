...
mkdir -p .claude/clio .claude/docs/tasks/feat .claude/docs/specs/memory
printf '| # | Task | Spec | Status |\n|---|---|---|---|\n| 1 | x | m | ✅ |\n| 2 | y | m | ⚠️ open |\n' > .claude/docs/specs/requirements.md
touch .claude/docs/specs/memory/m.md
mkdir -p .claude/docs/plans
printf '| # | Task | req | Test | Needs | Done |\n|---|---|---|---|---|---|\n| 1.1 | login | 1 | `go test` | – | [ ] |\n| 1.2 | logout | 1 | `go test` | 1.1 | [ ] |\n' > .claude/docs/plans/acct.md
A=.claude/docs/tasks/feat/1700000001_alpha.md
B=.claude/docs/tasks/feat/1700000002_beta.md
L=.claude/docs/tasks/legacy.md                       # pre-3.0 flat doc, no id — still readable
touch "$A" "$B" "$L" .claude/docs/tasks/feat/summary.md
...
idx 1700000009 .claude/docs/tasks/feat/1700000009_gone.md '[]' '[]' >> $I; ko index "doc missing on disk"
...
# all: a migrated doc — pre-3.0 records still key on the old path, which /clio:audit has moved away.
# The record carrying `supersedes` is what resolves them; without it this is a FAIL, with it an INFO.
mkdir -p .claude/docs/tasks/login
M=.claude/docs/tasks/login/1700000005_login.md; touch "$M"
old .claude/docs/tasks/2026-08-01_login.md '["l.go"]' '[1]'  >  $I
echo '{"date":"2026-01-01","id":"1700000005","type":"task","doc":"'"$M"'","domain":"x","plan_tasks":[],"files":["l.go"],"commits":[],"keywords":["k"],"req":[1],"specs":[],"supersedes":".claude/docs/tasks/2026-08-01_login.md"}' >> $I
...
# they must not keep reporting the very fields the successor just supplied — otherwise /clio:audit
...
# HOP0's one-liner runs on every plan file. /clio:plan writes a table, but an area /clio:audit
# lifted verbatim out of requirements.md is still bullets until it is re-planned — and cutting
# columns out of a bullet prints the whole line, which is what a "where are we?" answer must not do.
nxt(){ local n; n=$(grep -m1 '\[ \]' "$1"); case $n in '|'*) n=$(cut -d'|' -f2,3,5 <<<"$n") ;; *) n=${n:0:80} ;; esac; tr -s ' ' <<<"$n"; }
mkdir -p .claude/docs/plans
printf '| # | Task | req | Test | Needs | Done |\n|---|---|---|---|---|---|\n| 3.1 | orders list | 3 | `go test ./orders` | - | [ ] |\n' > .claude/docs/plans/tbl.md
printf -- '- [x] **1.1** done thing. Test: t.\n- [ ] **1.16** File association registration for JPG/JPEG/PNG/WebP/GIF/TIFF/BMP, at least on your primary dev OS first, others tracked as follow-ups (req #30). Test: double-click.\n' > .claude/docs/plans/blt.md
t=$(nxt .claude/docs/plans/tbl.md); b=$(nxt .claude/docs/plans/blt.md)
grep -qF -- 'orders list' <<<"$t" || { echo "HOP0 next: lost the task on a table plan: $t"; exit 1; }
grep -qF -- 'go test ./orders' <<<"$t" || { echo "HOP0 next: lost the test on a table plan: $t"; exit 1; }
grep -qF -- '1.16' <<<"$b" || { echo "HOP0 next: lost the id on a bullet plan: $b"; exit 1; }
[ "${#b}" -le 84 ] || { echo "HOP0 next: dumped a whole bullet line (${#b} chars): $b"; exit 1; }
[ "$(printf '%s' "$b" | wc -l)" = 0 ] || { echo "HOP0 next: spilled onto several lines"; exit 1; }
rm -rf .claude/docs/plans/tbl.md .claude/docs/plans/blt.md
...
idx 1700000001 "$A" '["a.go"]' '[1]'                >  $I
debt a spec-blocked '[2]' '"PO owes tax rule"'      >  $D
printf 'see `.claude/docs/decisions/1700000099_gone.md` and `.claude/docs/tasks/<feature>/<id>_x.md`\n' >> "$A"
out=$(bash "$V" all) || { echo "a dead link must warn, not fail"; echo "$out"; exit 1; }
grep -q 'dead link in a document: .claude/docs/decisions/1700000099_gone.md' <<<"$out" \
